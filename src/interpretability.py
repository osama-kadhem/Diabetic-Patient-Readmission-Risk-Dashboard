...
def generate_stability_visuals(stability_df, topk_sets, output_dir="clinical_models/stability_report"):
    ...
    matrix = np.zeros((n, n))
    np.fill_diagonal(matrix, 1.0)  # Jaccard similarity with self is 1.0
    ...
    img_path = Path(output_dir) / "stability_heatmap.png"
    plt.savefig(img_path)
    plt.close()

    print(f"Generated stability heatmap: {img_path}")

    # Per-model feature importance charts
    for model_id in model_ids:
        df_topk = topk_sets.get(f"{model_id}_df")
        if df_topk is not None:
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.barh(df_topk['feature'], df_topk['importance'], color='#0284c7')
            ax.invert_yaxis()
...
def artifact_export_pack(stability_df, topk_sets, output_dir="clinical_models/stability_report"):
    """Saves final CSVs and README for the stability report."""
    os.makedirs(output_dir, exist_ok=True)

    stab_path = Path(output_dir) / "stability_metrics.csv"
    ...
    readme_content = f"""# Explanation Stability Report
...
Files included:
- `stability_heatmap.png`: Heatmap of pairwise Jaccard similarity.
- `stability_metrics.csv`: Raw pairwise stability metrics.
- `top10_<model>.csv`: Top driving features for each trained model version.
"""
    with open(Path(output_dir) / "README.md", 'w') as f:
        f.write(readme_content)

    print(f"Exported stability report to {output_dir}/")
